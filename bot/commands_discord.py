...
MIN = int(os.environ.get("BOT_MIN"))
...
async def r(ctx, max_: int = 100):
    ...
    if not os.path.exists(f"data/{ctx.guild.name}"):
        os.makedirs(f"data/{ctx.guild.name}")
...
@bot.command(
    name='clear_cheat',
    aliases=['cc'],
    description='Clear the cheating dictionary',
    help='Clear the cheating dictionary, like this !clear_cheat NAME, you also can use !cc NAME',
    hidden=True
)
async def clear_cheating_dictionary(ctx, name):
    if ctx.author.guild_permissions.manage_roles or discord.utils.get(ctx.author.roles, name=ROLE):
        if name in cheating_dictionary:
...
async def clear_image(ctx):
    if ctx.author.guild_permissions.manage_roles or discord.utils.get(ctx.author.roles, name=ROLE):
        ...
        for file_name in png_files:
            ...
            os.remove(file_path)
        os.remove(f"data/{ctx.guild.name}/statistic.png")
        await ctx.message.delete()
...
async def save(ctx):
    if ctx.author.guild_permissions.manage_roles or discord.utils.get(ctx.author.roles, name=ROLE):
        ...
        file_path = f"data/{ctx.guild.name}/save/" + folder_name
        ...
        try:
            ...
            for file_name in png_files:
                ...
                with open(f"data/{ctx.guild.name}/users/{file_name}", 'rb') as f:
                    image = f.read()
                with open(os.path.join(file_path, file_name), 'wb') as f:
                    f.write(image)

            with open(f"data/{ctx.guild.name}/statistic.png", 'rb') as f:
                image = f.read()
            with open(os.path.join(file_path, "statistic.png"), 'wb') as f:
                f.write(image)
            with open(f"data/{ctx.guild.name}/compare.png", 'rb') as f:
                image = f.read()
            with open(os.path.join(file_path, "compare.png"), 'wb') as f:
                f.write(image)
            shutil.copy2(f"data/{ctx.guild.name}/roll.csv", file_path)
...
async def plot(ctx, user: discord.User = None):
    try:
        if not os.path.exists(f"data/{ctx.guild.name}/users/"):
            os.makedirs(f"data/{ctx.guild.name}/users/")
        ctx_data = pd.read_csv(f"data/{ctx.guild.name}/roll.csv", sep=',')
        for names in ctx_data['Name']:
            plot_aux(ctx, names, ctx_data)

        x = send_to_discord(ctx)
        if user is not None:
            await ctx.send(file=discord.File(f"data/{ctx.guild.name}/users/{user.display_name}_plot.png"))
        else:
